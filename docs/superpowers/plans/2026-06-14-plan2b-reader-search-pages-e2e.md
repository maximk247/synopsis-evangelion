...
> **STATUS: COMPLETED 2026-06-14.** As-built deviations from the written tasks (this note is the source of truth where they differ):
> 1. **Two-project Vitest config (not single-env + per-file docblock).** Component tests need Svelte's *browser* build (else `mount` is unavailable on the server build). The config uses `test.projects`: a `client` project (jsdom + `@testing-library/svelte`'s `svelteTesting()` plugin, includes `*.svelte.{test,spec}.ts`) and a `server` project (node, the rest). `settings.svelte.test.ts` therefore runs in the client project — harmless (it has no DOM deps).
> 2. **`fireEvent` is imported from `@testing-library/svelte`** (re-export), not `@testing-library/dom` (not a direct dep). Component assertions use core matchers (`toBeTruthy`, `queryByRole().not.toBeNull()`), not `jest-dom`, so `svelte-check` stays clean without extra type wiring. `SettingsDrawer` is tested by passing a plain `open: true` prop (`$bindable` lets the component close itself locally).
> 3. **Playwright** uses chromium-headless-shell on port **4319** (`--strictPort`) to avoid a stale local preview; column-heading assertions use `exact: true` (the `<h1>` title can contain a gospel name, e.g. "…Иоанна Предтечи"). Result: **29 unit + 10 e2e green**, full static build OK.
